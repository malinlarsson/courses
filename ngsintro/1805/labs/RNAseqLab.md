...
To get going, let's book a node, create a working directory named _transcriptome_ in the _/proj/g2018009/nobackup/<username>/_ directory and link the raw sequencing files .fastq.gz. NB! Remember to replace <username> with your uppmax id throughout the exercise.
...
{% highlight bash %}
salloc -A g2018009 -t 08:00:00 -p core -n 5 --no-shell --reservation=g2018009_4 &
{% endhighlight %} 
</details>  
<details>
<summary>:key: Click to see how to book a node on Friday</summary>
{% highlight bash %}
salloc -A g2018009 -t 08:00:00 -p core -n 5 --no-shell --reservation=g2018009_5 &
{% endhighlight %} 
</details>  
<br />


:computer: **Create a folder** named _transcriptome_ for your project in your _/proj/g2018009/nobackup/\<username\>_ directory. **Create a sub-folder** called _DATA_.
<details>
...
<summary>:key: Click to see suggested commands</summary>
{% highlight bash %}
for i in /proj/g2018009/nobackup/<username>/transcriptome/DATA/* 
do 
  fastqc $i -o /proj/g2018009/nobackup/<username>/transcriptome/fastqc/ 
done
{% endhighlight %}
</details>  
<br />
...
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/reference
{% endhighlight %}
</details>  
<br />
...
{% highlight bash %}
cd /proj/g2018009/nobackup/<username>/transcriptome/reference
...
{% highlight bash %}
ll -h /proj/g2018009/nobackup/<username>/transcriptome/reference/
...
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/indexChr11
cd /proj/g2018009/nobackup/<username>/transcriptome/indexChr11
{% endhighlight %}
</details>  
<br />
...
{% highlight bash %}
star --runMode genomeGenerate --runThreadN 8 --genomeDir /proj/g2018009/nobackup/<username>/transcriptome/indexChr11 --genomeFastaFiles /proj/g2018009/nobackup/<username>/transcriptome/reference/Mus_musculus.GRCm38.dna.chromosome.11.fa --sjdbGTFfile /proj/g2018009/nobackup/<username>/transcriptome/reference/Mus_musculus.GRCm38.85.gtf
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
ll -h /proj/g2018009/nobackup/<username>/transcriptome/indexChr11
...
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/star
mkdir /proj/g2018009/nobackup/<username>/transcriptome/star/SRR3222409
{% endhighlight %}
</details>
<br />
...
<details>
 ...
 {% highlight bash %}
 star --genomeDir /proj/g2018009/nobackup/<username>/transcriptome/index/complete --readFilesIn /proj/g2018009/nobackup/<username>/transcriptome/DATA/SRR3222409_1.fastq.gz /proj/g2018009/nobackup/<username>/transcriptome/DATA/SRR3222409_2.fastq.gz --runThreadN 8 --readFilesCommand zcat --outFileNamePrefix /proj/g2018009/nobackup/<username>/transcriptome/star/SRR3222409/SRR3222409_
 {% endhighlight %}
 </details>
<br />
...
{% highlight bash %}
cp -r /sw/share/compstore/courses/ngsintro/rnaseq/main/star/SRR3222410/ /proj/g2018009/nobackup/<username>/transcriptome/star/
{% endhighlight %}
</details>
...
do
  cp -r /sw/share/compstore/courses/ngsintro/rnaseq/main/star/$i /proj/g2018009/nobackup/<username>/transcriptome/star/
done
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/bams
cd /proj/g2018009/nobackup/<username>/transcriptome/bams 
...
{% highlight bash %}
ln -s /proj/g2018009/nobackup/<username>/transcriptome/star/SRR3222409/SRR3222409_Aligned.out.sam
{% endhighlight %}
</details>
<details>
...
{% highlight bash %}
ll -h  /proj/g2018009/nobackup/<username>/transcriptome/bams/*.sam
...
{% highlight bash %}
cp /sw/share/compstore/courses/ngsintro/rnaseq/main/bams/out/SRR3222410_Aligned.out.bam /proj/g2018009/nobackup/<username>/transcriptome/bams/
{% endhighlight %}
</details>
<details>
...
{% highlight bash %}
for i in SRR3222411 SRR3222412 SRR3222413 SRR3222414
  do cp "/sw/share/compstore/courses/ngsintro/rnaseq/main/bams/out/"$i"_Aligned.out.bam" /proj/g2018009/nobackup/<username>/transcriptome/bams/ 
done
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
samtools sort -o /proj/g2018009/nobackup/<username>/transcriptome/bams/SRR3222409_Aligned.out.sorted.bam -T sort_temp /proj/g2018009/nobackup/<username>/transcriptome/bams/SRR3222409_Aligned.out.bam
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how to copy over sample by sample</summary>
{% highlight bash %}
cp /sw/share/compstore/courses/ngsintro/rnaseq/main/bams/out/SRR3222410_Aligned.out.sorted.bam /proj/g2018009/nobackup/<username>/transcriptome/bams/
{% endhighlight %}
</details>
<details>
...
{% highlight bash %}
for i in SRR3222411 SRR3222412 SRR3222413 SRR3222414 
  do cp "/sw/share/compstore/courses/ngsintro/rnaseq/main/bams/out/"$i"_Aligned.out.sorted.bam" /proj/g2018009/nobackup/<username>/transcriptome/bams/
done
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
ll -h  /proj/g2018009/nobackup/<username>/transcriptome/bams/*.sorted.bam
...
{% highlight bash %}
samtools index /proj/g2018009/nobackup/<username>/transcriptome/bams/SRR3222410_Aligned.out.sorted.bam
{% endhighlight %}
</details>
<details>
<summary>:key: Click to see how to index BAM file, using bash loop</summary>
{% highlight bash %}
for i in  /proj/g2018009/nobackup/<username>/transcriptome/bams/*.sorted.bam
  do samtools index $i
done
{% endhighlight %}
</details>
...
{% highlight bash %}
ll -h  /proj/g2018009/nobackup/<username>/transcriptome/bams/*.bai
...
<summary>:key: Click to see the suggested commands</summary>
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/qualimap 
cd /proj/g2018009/nobackup/<username>/transcriptome/qualimap/
module load QualiMap/2.2
{% endhighlight %}
</details>
<br />

:computer: **Run QualiMap** for the fist sample on sorted BAM file *SRR3222409\_Aligned.out.sorted.bam* **directing** the results to */proj/g2018009/nobackup/<username>/transcriptome/qualimap/SRR3222409* folder. :bulb: QualiMap creates the folder if you specify the right parameter
<details>
<summary>:key: Click to see the suggested commands</summary>
{% highlight bash %}
qualimap rnaseq -pe -bam /proj/g2018009/nobackup/<username>/transcriptome/bams/SRR3222409_Aligned.out.sorted.bam -gtf /proj/g2018009/nobackup/<username>/transcriptome/reference/Mus_musculus.GRCm38.85.gtf --outdir /proj/g2018009/nobackup/<username>/transcriptome/qualimap/SRR3222409 --java-mem-size=63G > /dev/null 2>&1
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
ll -h /proj/g2018009/nobackup/<username>/transcriptome/qualimap/SRR3222409/
...
{% highlight bash %}
cp -r /sw/share/compstore/courses/ngsintro/rnaseq/main/qualimap/SRR3222410 /proj/g2018009/nobackup/<username>/transcriptome/qualimap/
{% endhighlight %}
</details>
<details>
...
for i in SRR3222409 SRR3222410 SRR3222411 SRR3222412 SRR3222413 SRR3222414
  do cp -r "/sw/share/compstore/courses/ngsintro/rnaseq/main/qualimap"/$i /proj/g2018009/nobackup/<username>/transcriptome/qualimap/
done
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how...</summary>
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/featurecounts
cd /proj/g2018009/nobackup/<username>/transcriptome/featurecounts
{% endhighlight %}
</details>
<br />
...
:computer: **Sym-link** SAM files generated by Star and located in */proj/g2018009/nobackup/<username>/transcriptome/star*
<details>
<summary>:key: Click to see sym-link files, sample by sample</summary>
{% highlight bash %}
ln -s /proj/g2016035/nobackup/<username>/transcriptome/star/SRR3222409/SRR3222409_Aligned.out.sam
{% endhighlight %}
</details>
<details>
...
{% highlight bash %}
featureCounts -p -a /proj/g2018009/nobackup/<username>/transcriptome/reference/Mus_musculus.GRCm38.85.gtf -t gene -g gene_id -s 0 -o tableCounts *.sam
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how </summary>
{% highlight bash %}
mkdir /proj/g2018009/nobackup/<username>/transcriptome/DE 
cd /proj/g2018009/nobackup/<username>/transcriptome/DE
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how </summary>
{% highlight bash %}
ln -s /proj/g2018009/nobackup/<username>/transcriptome/featurecounts/tableCounts
...
<details>
 ...
 {% highlight bash %}
 scp -r <your_username>@rackham.uppmax.uu.se:/sw/share/compstore/courses/ngsintro/rnaseq/bonus/funannot ./
 {% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see an example of a command</summary>
{% highlight bash %}
cp -r /sw/share/compstore/courses/ngsintro/rnaseq/bonus/funannot/ /proj/g2018009/nobackup/<username>/transcriptome/
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how</summary>
{% highlight bash %}
cd /proj/g2018009/nobackup/<username>/transcriptome/funannot
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see an example of a command</summary>
{% highlight bash %}
cp -r /sw/share/compstore/courses/ngsintro/rnaseq/bonus/exon /proj/g2018009/nobackup/<username>/transcriptome
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see an example of a command</summary>
{% highlight bash %}
scp -r YOUR_LOGIN@rackham.uppmax.uu.se:/sw/share/compstore/courses/ngsintro/rnaseq/bonus/exon ./
{% endhighlight %}
</details>
<br />
...
<summary>:key: Click to see how</summary>
{% highlight bash %}
cd /proj/g2018009/nobackup/<username>/transcriptome/exon
{% endhighlight %}
</details>
<br />
...
{% highlight bash %}
scp username@rackham.uppmax.uu.se:/proj/g2018009/nobackup/<username>/transcriptome/bamfile.bam .
{% endhighlight %}
...
:computer: Method 2. Go to [rackham-gui](rackham)
...
{% highlight bash %}
scp username@rackham.uppmax.uu.se:/proj/g2018009/nobackup/<username>/transcriptome/DE/coverage.pdf .
{% endhighlight %}
